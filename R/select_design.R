...
select_design <- function(geometry, distance, n){
  if (!all(geometry %in% c("simplex", "hypercube"))){
    stop(paste0("Geometry should be either 'simplex' or 'hypercube', not ", geometry))
  }
  if (!all(distance %in% c("manhattan", "chebyshev", "euclidean"))){
    stop(paste0("Distance should be either 'manhattan', 'chebyshev', or 'euclidean', not ", distance))
  }
